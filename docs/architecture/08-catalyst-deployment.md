...
| --- | --- | --- | --- |
| 2026-06-29 | `31210000000317080` | Prod 0629 2 toasts + sound desktop 1.5.3 | **1 entity, no gate.** Web client only — completion toasts collapse to two (success `Dossier ready` + failure `Generation failed`; the two `partial` warning toasts removed, `partial` now shows the success toast), and the success chime is reserved for a desktop browser tab (silenced on mobile and inside an installed PWA — reverses the 1.5.2 mobile sound toggle; desktop/OS notification unchanged on all platforms). Diff verified **Total Changes: 0** for Serverless, Data Store, NoSQL, File Store, Stratus, Cron, Job Pool, API Gateway, Cache, and every auth component — only `Web Client Hosting` Updated:1. All 25 components completed, empty error logs. App changelog `1.5.3`. |
| 2026-06-27 | `31210000000296174` | Prod 0627 mobile + PWA icon v1.5.2 | **1 entity, fully additive, no gate.** Web client only — home-screen/PWA icon (apple-touch-icon PNG + web manifest using the ELISS logo), mobile completion-alert toggles in the bottom-sheet menu, mobile PDF via the native share sheet (fixes iOS "unknown.pdf"), bottom-center mobile toasts, and the Settings save-bar safe-area. Diff verified **Total Changes: 0** for Serverless, Data Store, NoSQL, File Store, Stratus, Cron, Job Pool, API Gateway, Cache, and every auth component. App changelog `1.5.2`. |
| 2026-06-26 | `31210000000296133` | Prod 0626 leads whole-row click v1.5.1 | **1 entity, fully additive, no gate.** Web client only — desktop Leads-table rows now open the dossier on a click anywhere in the row (name stays a link; inner controls / modifier-clicks / text selection ignored). Diff verified **Total Changes: 0** for Serverless, Data Store, NoSQL, File Store, Stratus, Cron, Job Pool, API Gateway, Cache, and every auth component. App changelog `1.5.1`. |
