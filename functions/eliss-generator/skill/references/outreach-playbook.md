...
### Slot 3 — Always Insight Drop

Slot 3 is **always** the **Insight Drop / The One Idea** template. Executive voice, Steve Jobs cadence.
Not a follow-up. Not a breakup. A single distilled observation that reframes
how the prospect sees their own situation — earned from a specific dossier
fact, not a generic platitude. The email IS the gift; there is no CTA.
Do not skip — a well-earned insight converts better than another ask.
...
moves to slot 2 (displacing the LLM pick). Slot 3 stays Insight Drop.
...
### #9 — Insight Drop / The One Idea (Executive voice, Steve Jobs cadence) — ALWAYS slot 3

- **Triggered by:** always — slot 3 is reserved.
- **Subject pattern:** a single declarative observation. *"The thing about \<their compliance regime\>"* / *"Identity is the new perimeter"* / *"Why \<incumbent\> can't see what's coming"*. Never a question. Never "Closing the loop." Never "Last note."
- **Body anchors (3–5 sentences total — no more):**
  1. **The reframe**: open by pulling the prospect's specific situation up to a higher-altitude insight. Most people see X; the second-order truth is Y. Ground the reframe in a concrete dossier fact (compliance regime, incumbent stack, leadership change, regulatory deadline) — never a generic industry platitude.
  2. **The crystallized claim**: one sentence that lands the idea. Declarative. No qualifiers. *"Identity, not the network, is the only perimeter that survives a multi-cloud failover."* / *"The DORA Article 17 clock starts on detection, not breach — that flips the SIEM equation."*
  3. **The (optional) asymmetry**: one sentence noting what most peers in their cohort are missing about this. Not boastful; observational.
- **Hard rules:**
  - **No CTA.** The insight is the gift. No "happy to chat", no "20 minutes", no link, no calendar.
  - **No product pitch.** AD360 / Log360 are not named. The reader closes the email thinking about *their problem*, not your product.
  - **Earned, not generic.** Every Insight Drop must reference a specific dossier fact. "AI is reshaping cybersecurity" is LinkedIn slop. "Your DORA Article 17 2-hour clock means detection-time is now a board-level metric" is an Insight Drop.
  - **Jobs cadence:** declarative sentences, white space, no bullet lists, no headers. Read it aloud — it should sound like one person thinking out loud, not a marketing email. Under 80 words.
  - **Sign-off is just the rep's first name.** No signature, no title, no link.
...
| `template_id` | string | ✅ | one of `compliance_gap`, `renewal_window`, `breach_incident`, `competitor_displacement`, `peer_benchmark`, `technical_deep_dive`, `org_change`, `cost_consolidation`, `insight_drop`, `hybrid_cloud_migration`, `audit_deadline`, `executive_briefing_offer`, `event_followup` |
...
```json
{
  "recommended_outreach": [
    ...
    { "slot": 3, "template_id": "insight_drop", "...": "..." }
  ]
}
```
